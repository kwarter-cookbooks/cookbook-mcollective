module MCollective
  module Agent
    class Supervisor<RPC::Agent

      action 'stop' do
        stop_result = Supervisor.do_service_action('stop', request[:service])
      ...
      action 'start' do
        start_result = Supervisor.do_service_action('start', request[:service])
      ...
      action 'restart' do
        reply[:status] = Supervisor.do_service_action('restart', request[:service])
      end

      action 'status' do
        reply[:status] = Supervisor.do_service_action('status', request[:service])
      end
      ...
      def self.do_service_action(action, service)
        ...
        provider = @config.pluginconf.fetch('service.provider', 'supervisor')
        provider_options = {}
