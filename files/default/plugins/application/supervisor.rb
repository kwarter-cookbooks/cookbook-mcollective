module MCollective
  class Application::Supervisor < Application
    description "Manages system services"

    usage <<-END_OF_USAGE
mco supervisor <SERVICE> <ACTION>
...
END_OF_USAGE
    ...
    def main
      ...
      service = rpcclient('supervisor')
      service_result = service.send(configuration[:action], :service => configuration[:service])

      sender_width = service_result.map{|s| s[:sender]}.map{|s| s.length}
